    ...
    private fun initInfoBtnsListener() {
        with(binding) {
            btnTermsOfService.setOnSingleClickListener {
                startActivity(Intent(Intent.ACTION_VIEW, Uri.parse(WEB_TERMS_OF_SERVICE)))
            }
            btnPrivacyPolicy.setOnSingleClickListener {
                startActivity(Intent(Intent.ACTION_VIEW, Uri.parse(WEB_PRIVACY_POLICY)))
            }
            btnCompanyInfo.setOnSingleClickListener {
                startActivity(Intent(Intent.ACTION_VIEW, Uri.parse(WEB_COMPANY_INFO)))
            }
